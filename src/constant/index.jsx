import user1 from "../assets/image/opara.png";
import user4 from "../assets/image/titcom.png";
import user5 from "../assets/image/adeniyi.png";
import user5a from "../assets/image/adeniyi-1.png";
import user5b from "../assets/image/adeni-2.png";
import user6 from "../assets/image/fatunbi.jpg";
import user6a from "../assets/image/fatunbi-wif.png";
import user6b from "../assets/image/fatunbi-b.png";
import user9 from "../assets/image/ogundipeW.png";
import user9a from "../assets/image/ogun-1.png";
import user10 from "../assets/image/fabgenro-1.png";
import user10c from "../assets/image/fagbenro2.png";
import user10b from "../assets/image/fagbenro-w.png";
import user11b from "../assets/image/oyibo-m.png";
import user11 from "../assets/image/oyinbo.png";
import user11a from "../assets/image/oyibo-wife.png";
import user0 from "../assets/image/taiwonew.png";
import user13 from "../assets/image/patrick.png";
import user13b from "../assets/image/patrick-b.png";
import user13a from "../assets/image/onabule-a.png";
import user14 from "../assets/image/roo.png";
import user14b from "../assets/image/roland-w.png";
import user14a from "../assets/image/rolland-t.png";
import user15 from "../assets/image/lekan1.png";
import user15b from "../assets/image/lekan-w.png";
import user15a from "../assets/image/lekan-t.png";
import user16 from "../assets/image/adedamola.jpg";
import user16b from "../assets/image/adedamolawife.jpg";
import user16a from "../assets/image/adejobi-w.png";
import user17 from "../assets/image/olufu.png";
import user17b from "../assets/image/olufe-b.png";
import user17a from "../assets/image/olofu-wife.png";
import user18 from "../assets/image/felix-1.png";
import user18a from "../assets/image/felix-t.png";
import user18b from "../assets/image/felix-w.png";
import user19 from "../assets/image/adigun.png";
import user19b from "../assets/image/adigun-t.png";
import user19a from "../assets/image/adigun-w.png";
import user20 from "../assets/image/aregbesola.png";
import user14e from "../assets/image/isehww.png";

import user21 from "../assets/image/adegoke-1.png";
import user21b from "../assets/image/adegoke-2.png";
import user21w from "../assets/image/adegoke-Wife.png";

import cor1 from "../assets/image/Mr._George_Diyaolu.png";
import cor1b from "../assets/image/diyawife.png"; // president wife to be change
import cor2 from "../assets/image/Mr. Dennis Oni.jpg";
import cor2b from "../assets/image/denis-wife.png";
import cor2a from "../assets/image/oni2a.png";
import cor3 from "../assets/image/frances.png";
import cor3b from "../assets/image/francis-b.png";
import cor3a from "../assets/image/irepe.png";

import wf1 from "../assets/image/diyaolu-t.png";
import wf1a from "../assets/image/diyaolu-wife1.png";
import wf2 from "../assets/image/Mr. & Mrs. Taiwo Ogunfolaji.jpg";
import wf4 from "../assets/image/opara_wife.png";
import wf5 from "../assets/image/titcom-b.png";
import wf5b from "../assets/image/titcomWf.png";
import wf4a from "../assets/image/opara-1.png";
import wf7 from "../assets/image/rose_mary.png";
import wf7b from "../assets/image/Olatunji22.png";
import wf8 from "../assets/image/funmilay.png";
import wf9 from "../assets/image/mrsaregbesola.png";
import wf10 from "../assets/image/paul_taiwo.png";
import wf10b from "../assets/image/paul_taiwoB.png";
import wf10c from "../assets/image/paul_taiwoW.png";
import wf11 from "../assets/image/akindele.png";
import wf11b from "../assets/image/akindeleB.png";
import wf11c from "../assets/image/akindeleC.png";
import wf12 from "../assets/image/fatona.png";
import wf12b from "../assets/image/fatonaB.png";
import wf12c from "../assets/image/fatonac.png";


import pst1 from "../assets/image/timoti.jpg";
import pst2 from "../assets/image/Peter Kayode.png";
import pst3 from "../assets/image/Thomas.png";
import pst4 from "../assets/image/stanly-A.png";
import pst4b from "../assets/image/stanley-B.png";
import pst4c from "../assets/image/aikulolawm.png";
import pst6 from "../assets/image/olutan.png";
import pst5 from "../assets/image/default.jpg";
import pst7 from "../assets/image/olotu.png";
import pst7b from "../assets/image/OlutanWf.png";
import pst7c from "../assets/image/olutan2.png";


import pst8 from "../assets/image/opadijo.png";





//==========================the event images=================================================


import bg3 from "../assets/image/bg3.jpg";
import bg2 from "../assets/image/b2.jpg";

// =======================================================================================

// triumphant
import m1 from "../assets/image/olotu.png";

export const navItem = [
  { label: "Home", href: "/" },
  { label: "Active Members", href: "/current" },
  { label: "Past Presidents", href: "/past" },
  { label: "Member Triumphant", href: "/triumphant" },
];

export const namesOne = [
  "Segun Ogunfolaji",
  "Peter Bankole",
  "Dayo Akindele",
  "Henry Lawrence",
  "Titus Niniola",
  "Peter Olayemi",
  "Blessing Sobayo",
  "Segun Sobayo",
  "Niyi Osikoya",
  "Femi Osikoya",
  "Phillip Opadijo",
  "Achike Louis",
  "Ranti Diyaolu",
  "Niyi Alli",
  "Felix Ohia",
  "John Ogundipe",
  "Felix Oyebola",
  "Yemi Alex Idowu",
  "Stephen Adeniran",
  "N. Kehinde",
  "Otunba Anthony Alli",
];

export const nameTwo = [
  { name: "Mr. Henry Lawrence", position: "President" },
  { name: "Mr. Segun Ogunfolaji", position: "Vice President" },
  { name: "Mr. Peter Bankole", position: "General Secretary" },
  { name: "Mr. Femi Osikoya", position: "Asst. Secretary" },
  { name: "Mr. Titus Niniola", position: "Treasurer" },
  { name: "Mr. Louis Achike", position: "Financial Secretary" },
  { name: "Mr. Ranti Diyaolu", position: "Social Secretary" },
];

export const nameThree = [
  "Ranti Diyaolu",
  "Segun Ogunfolaji",
  "Peter Bankole ",
  "Henry Lawrence",
];

export const aims = [
  {
    obj: "To promote Christianity within and outside the Catholic Church.",
    icon: <i className="fa-solid fa-cross"></i>,
  },
  {
    obj: "To foster cooperation and love among the members and their families.",
    icon: <i className="fa-regular fa-heart"></i>,
  },
  {
    obj: "To organize educational and recreational activities for members and their families from time to time.",
    icon: <i className="fa-solid fa-graduation-cap"></i>,
  },
];

export const clubs = [
  {
    obj: "To promote Christianity within and outside the Catholic Church.",
    icon: <i className="fa-solid fa-cross"></i>,
  },
  {
    obj: "To foster cooperation and love among the members and their families.",
    icon: <i className="fa-regular fa-heart"></i>,
  },
  {
    obj: "To organize educational and recreational activities for members and their families from time to time.",
    icon: <i className="fa-solid fa-graduation-cap"></i>,
  },
  {
    obj: "To cater for the welfare of the members.",
    icon: <i className="fa-solid fa-user-group"></i>,
  },
  {
    obj: "To take active part in all church activities",
    icon: <i className="fa-solid fa-church"></i>,
  },
  {
    obj: "To initiate and support developmental programmes in the church.",
    icon: <i className="fa-solid fa-handshake-angle"></i>,
  },
];

export const members = [
  { name: "Mr. Femi Kalesanwo", image: pst5,  },
  { name: "Mr. Phillip Opadijo", image: pst8,},
  { name: "Mr. JK Diyaolu", image: pst5, },
  { name: "Mr. FI Agu", image: pst5, },
  { name: "Mr. JT Olatunde", image: pst5, },
  { name: "Mr. Martins Osita Anigwe", image: pst5, },
  { name: "Olootu Patrick Olusegun Ogunfolaji", image: m1, },
];

export const current_exco = [
  {
    name: "Mr.George Oludare Diyaolu",
    name2: "Mrs Susannah Modupeola Diyaolu",
    image: cor1,
    position: "President",
    wifeImage1: cor1b,
    wifeImage2: wf1,
  },
  {
    name: "Mr.Taiwo Michael Ogunfolaji",
    name2: "Mrs.Josephine Mena Ogunfolaji",
    image: user0,
    position: "Vice President",
    wifeImage1: wf1a,
    wifeImage2: wf2,
  },

  {
    name: "Mr.James Olusegun Adeniyi",
    name2: "Mrs. Alice Oluyemisi Adeniyi",
    image: user5,
    position: "Secretary",
    wifeImage1: user5a,
    wifeImage2: user5b,
  },
  {
    name: "Mr. Martin Ogagaoghene Oyibo",
    name2: "Mrs. Mary Orokpo Oyibo",
    image: user11b,
    position: "Assistant Secretary",
    wifeImage1: user11a,
    wifeImage2: user11,
  },

  {
    name: "Mr. Felix Ayoola Alajogun.",
    name2: "Mrs. Latifat Faderera Alajogun",
    image: user18,
    position: "Treasurer",
    wifeImage1: user18a,
    wifeImage2: user18b,
  },

  {
    name: "Mr. Dennis Oyewunmi Oni",
    name2: "Mrs. Janet Omolola Oni",
    image: cor2,
    position: "Financial Sectary",
    wifeImage1: cor2a,
    wifeImage2: cor2b,
  },

  {
    name: "Mr. Michael Adeolu Fagbenro",
    name2: "Mrs. Olufunmilayo Elizabeth  Fagbenro",
    image: user10,
    position: "Social",
    wifeImage1: user10b,
    wifeImage2: user10c,
  },

  {
    name: "Architect. Francis Olamilekan Oliyide",
    name2: "Mrs Francisca Abimbola Oliyide", //wife name
    image: user15,
    position: "Chief Whip",
    wifeImage1: user15b,
    wifeImage2: user15a,
  },

  // ====================Active members================================

  {
    name: "Mr. Thomas Babatunde Aregbesola B.Sc,MBA,FCNA,FCTI,FCAI,M.IoD",
    name2: "Mrs. Elizabeth Olufunmilayo Aregbesola",
    image: user20,
    position: "Past President",
    wifeImage1: wf8,
    wifeImage2: wf9,
  },

  {
    name: "Mr. Timothy Ayoola Olatunji",
    name2: "Mrs. Rosemary Olatunji",
    image: pst1,
    position: "Active Member",
    wifeImage1: wf7,
    wifeImage2: wf7b,
  },
  {
    name: "Dr.Stanley Olu Ola Aikulola MNPOM",
    name2: "Mrs. Folake Aikulola",
    image: pst4,
    position: "Past President",
    wifeImage1: pst4b,
    wifeImage2: pst4c,
  },

  {
    name: "Mr. Gabriel Ajani Oladeji Olutan",
    position: "President",
    image: pst6,
    tiem: "2014 - 2018",
    name2: "Mrs. Mary Ayobami Olutan",
    wifeImage1: pst7b,
    wifeImage2: pst7c,
  },

  {
    name: "Mr. Peter Olukayode Adigun",
    name2: "Mrs. Mary Olayinka Adigun",
    image: user19,
    position: "Past President",
    wifeImage1: user19b,
    wifeImage2: user19a,
  },
  {
    name: "Mr. Anthony Oludayo Akindele",
    name2: "Mrs. Anthonia Olubunmi Akindele",
    image:wf11,
    position: "Active Member",
    wifeImage1: wf11b,
    wifeImage2: wf11c,
  },

  {
    name: "Mr. Lawrence Omotunde Owosina ",
    name2: "Mrs. Agnes Omolara Owosina",
    image: pst5,
    position: "Active Member",
    wifeImage1: pst5,
    wifeImage2: pst5,
  },

  {
    name: "Mr. Anthony Adeoye Adegoke ",
    name2: "Mrs. Titilayo Patience Adegoke",
    image: user21,
    position: "Active Member",
    wifeImage1: user21b,
    wifeImage2: user21w,
  },

  {
    name: "Mr. Rowland Ashiemamho Iseh",
    name2: "Mrs. Margaret Emoshiogue Iseh ",
    image: user14,
    position: "Active Member",
    wifeImage1: user14e,
    wifeImage2: user14a,
  },

  {
    name: "Mr. Ejeh Stephen Olofu",
    name2: "Mrs. Agnes Olofu",
    image: user17,
    position: "Active Member",
    wifeImage1: user17a,
    wifeImage2: user17b,
  },
 
  {
    name: "Mr. Adedamola Adejobi",
    name2: "Mrs. Olusola Rita Adejobi",
    image: user16,
    position: "Active Member",
    wifeImage1: user16a,
    wifeImage2: user16b,
  },

  {
    name: "Mr. Anthony Oluwaseyi Ogundipe",
    name2: "Mrs. Folake Rachael Ogundipe", //wife name
    image: user9a,
    position: "Active Member",
    wifeImage1: user9, // the wife piicture
    wifeImage2: user9,
  },

  {
    name: "Mr. Patrick Olayiwola Onabule, FCA",
    name2: "Mrs. Francisca Abosede Onabule, CNA, ACIT",
    image: user13,
    position: "Active Member",
    wifeImage1: user13a,
    wifeImage2: user13b,
  },

  {
    name: "Mr. Timothy Olaleye",
    name2: "Mrs. Olaleye",
    image: pst5,
    position: "Active Member",
    wifeImage1: pst5,
    wifeImage2: pst5,
  },

  {
    name: "Mr. Edward Adegboyega Titcombe",
    name2: "Mrs. Maria Ayodele Titcombe ",
    image: user4,
    position: "Active Member",
    wifeImage1: wf5b,
    wifeImage2: wf5,
  },
 
  {
    name: "Mr. Paul Taiwo Adigun",
    name2: "Mrs. Mary Taiwo Adigun",
    image: wf10,
    position: "Active Member",
    wifeImage1: wf10b,
    wifeImage2: wf10c,
  },

  {
    name: "Mr.Francis Imhoede Irekpita MNIQS, RQS",
    name2: "Mrs.Francisca Otseda Irekpita",
    image: cor3,
    position: "Active Member",
    wifeImage1: cor3a,
    wifeImage2: cor3b,
  },
  {
    name: "Prince. Felix Temitope Fatona",
    name2: "Mrs. Mary Arinola Fatona",
    image: wf12,
    position: "Active Member",
    wifeImage1: wf12b,
    wifeImage2: wf12c,
  },

  {
    name: "TPL. Michael Akanbi Fatunbi",
    name2: "Mrs. Victoria Adetola Fatunbi",
    image: user6,
    position: "Active Member",
    wifeImage1: user6a,
    wifeImage2: user6b,
  },

  {
    name: "Mr. George Uwaoma Opara",
    name2: "Mrs. Comfort Idongesit Opara",
    image: user1,
    position: "Active Member",
    wifeImage1: wf4a,
    wifeImage2: wf4,
  },
  // =============
 
];

export const past_member = [
  {
    name: "Mr. George Diyaolu",
    position: "Current President",
    image: cor1,
    tiem: "2021 till-date",
    wifeImage1: cor1b,
    name2: "Mrs Diyaolu",
  },

  {
    name: "Mr. Peter OluKayode Adigun",
    position: "President",
    image: pst2,
    tiem: "2018 - 2021",
    name2: "Mrs. Mary Olayinka Adigun",
    wifeImage1: user19a,
  },
  {
    name: "Mr. Gabriel Ajani Oladeji Olutan",
    position: "President",
    image: pst6,
    tiem: "2014 - 2018",
    name2: "Mrs. Mary Ayobami Olutan",
    wifeImage1: pst7b,
  },
  {
    name: "Dr. Stanley Olu Ola Aikulola",
    position: "Presidet",
    image: pst4,
    tiem: "2010 - 2014",
    name2: "Mrs. Josephine Olufolake Aikulola",
    wifeImage1: pst4b,
  },
  {
    name: "Mr. Timothy Ayoola Olatunji FICA, MCIPM, JP",
    position: "President",
    image: pst1,
    tiem: "2006 - 2010 ",
    name2: "Mrs. Rosemary Olatunji",
    wifeImage1:wf7,
  },

  {
    name: "Mr. Thomas Aregbesola",
    position: "President",
    image: user20,
    tiem: "2002 - 2006",
    name2: "Mrs. Funmilola Aregbesola",
    wifeImage1:wf8,
  },
  {
    name: "Mr Dele Adewunmi",
    position: "President",
    image: pst5,
    tiem: "1998 - 2000",
    name2: "",
    wifeImage1: "",
  },
  {
    name: "Mr A.K.A Odeyinka",
    position: "President",
    image: pst5,
    tiem: "1994 - 1996",
    name2: "",
    wifeImage1: "",
  },
  {
    name: "Mr Femi Kalesanwo",
    position: "President",
    image: pst5,
    tiem: "1990 - 1994",
    name2: "",
    wifeImage1: "",
    name2: "",
    wifeImage1: "",
  },
  {
    name: "Mr Sofela Coker",
    position: "President",
    image: pst5,
    tiem: "1986 - 1988",
    name2: "",
    wifeImage1: "",
  },
  {
    name: "Late Olootu Patrick Olusegun Ogunfolaji",
    position: "President",
    image: pst7,
    tiem: "1984 - 1986",
    name2: "",
    wifeImage1: "",
  },
  {
    name: "Mr Henry Ibiyemi Lawrence",
    position: "President",
    image: pst5,
    tiem: "1983 - 1984",
    name2: "",
    wifeImage1: "",
  },
];


// =============================================excurssion=========

import bg33 from '../assets/image/pr3.jpg';
import bg22 from '../assets/image/pr2.jpg';
import bg44 from '../assets/image/pr.jpg';
import bd from '../assets/image/badagry.jpg';
import bd2 from '../assets/image/badagry2.jpg';
import bd3 from '../assets/image/badagry3.jpg';
import ib from '../assets/image/ibadan.jpg';
import aw from '../assets/image/award.jpg';
import aw2 from '../assets/image/award2.jpg';
import ch from '../assets/image/ch.jpg';
import ch2 from '../assets/image/ch2.jpg';
import ch3 from '../assets/image/ch3.jpg';
import st from '../assets/image/st2.jpg';
import st2 from '../assets/image/st3.jpg';
import st3 from '../assets/image/st.jpg';
import ra from '../assets/image/ra.jpg';
import ra2 from '../assets/image/ra2.jpg';
import ra3 from '../assets/image/ra3.jpg';
import es from '../assets/image/es.jpg';
import es2 from '../assets/image/es2.jpg';
import es3 from '../assets/image/es3.jpg';

export const sections = [
  {
    id: 1,
    title: "Presidential nite ",
    date: "December 2021",
    fulld:"",
    images: [bg33,bg22,bg44],
  },
  {
    id: 2,
    title: "Family excursion to UI zoo",
    date: "",
    fulld:"",
    images: [ib],
  },
  {
    id: 3,
    title: "Family excursion to Badagry",
    date: "",
    fulld:" ",
    images: [bd,bd2,bd3],
  },
  {
    id: 4,
    title: "Productivity Merit award recieved by  ",
    date: "Dr.Stanley Olu Ola Aikulola",
    fulld:" ",
    images: [aw,aw2],
  },
  {
    id: 4,
    title: "Christmas Party, Fun Turf, MKO Abiola Stadium, ",
    date: "Abeokuta, December 2016.",
    fulld:" ",
    images: [ch,ch2,ch3],
  },
  {
    id: 4,
    title: "St peter feast day celebration",
    date: "Olomore ",
    fulld:" ",
    images: [st,st2,st3],
  },
  {
    id: 5,
    title: "St peter feast day celebratin",
    date: "Ibara ",
    fulld:" ",
    images: [ra,ra2,ra3],
  },
  {
    id: 6,
    title: "Famly retreat at Ogun golf course and resort center",
    date: "Okemosan,Abeokuta ",
    fulld:" ",
    images: [es,es2,es3],
  },
  
];