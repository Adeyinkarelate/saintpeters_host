import user1 from '../assets/image/Mr. George Opara.jpg'
import user2 from '../assets/image/Mrs. Thomas Aregbesola.jpg'
import user3 from '../assets/image/Mrs. Comfort Opara.jpg'
import user4 from '../assets/image/Mr. Titicom Adegboyega.jpg'
import user5 from '../assets/image/Mr. Segun Adeniyi.jpg'
import user6 from '../assets/image/Mr. Michael Fatumbi.jpg'
import user7 from '../assets/image/Mr. and Mrs. Onabule.jpg'
import user8 from '../assets/image/Mr. and Mrs. Ogunfolaji.jpg'
import user9 from '../assets/image/Mr. and Mrs. Ogundipe.jpg'
import user10 from '../assets/image/Mr. and Mrs Fagbenro.jpg'
import user11 from '../assets/image/Mr. & Mrs. Oyibo.jpg'
import user0 from '../assets/image/Mr. Taiwo Ogunfolaji.jpg'
import user12 from '../assets/image/Arctech and Mrs. Oluyide.jpg';

import cor1 from '../assets/image/Mr. George Diyaolu.jpg'
import cor2 from '../assets/image/Mr. Dennis Oni.jpg'
import cor3 from '../assets/image/Mr. Francis Irepita.jpg'

export const navItem = [
  { label: "Home", href: "/" },
  { label: "Current Executive", href: "/current" },
  { label: "Past Executives", href: "/past" },
  { label: "Active Members", href: "/active" },
];

export const namesOne = [
  "Segun Ogunfolaji",
  "Peter Bankole",
  "Dayo Akindele",
  "Henry Lawrence",
  "Titus Niniola",
  "Peter Olayemi",
  "Blessing Sobayo",
  "Segun Sobayo",
  "Niyi Osikoya",
  "Femi Osikoya",
  "Phillip Opadijo",
  "Achike Louis",
  "Ranti Diyaolu",
  "Niyi Alli",
  "Felix Ohia",
  "John Ogundipe",
  "Felix Oyebola",
  "Yemi Alex Idowu",
  "Stephen Adeniran",
  "N. Kehinde",
  "Otunba Anthony Alli",
];

export const nameTwo = [
  { name: "Mr. Henry Lawrence", position: "President" },
  { name: "Mr. Segun Ogunfolaji", position: "Vice President" },
  { name: "Mr. Peter Bankole", position: "General Secretary" },
  { name: "Mr. Femi Osikoya", position: "Asst. Secretary" },
  { name: "Mr. Titus Niniola", position: "Treasurer" },
  { name: "Mr. Louis Achike", position: "Financial Secretary" },
  { name: "Mr. Ranti Diyaolu", position: "Social Secretary" },
];

export const nameThree = [
  "Ranti Diyaolu",
  "Segun Ogunfolaji",
  "Peter Bankole ",
  "Henry Lawrence",
];

export const aims = [
  {
    obj: "To promote Christianity within and outside the Catholic Church.",
    icon: <i className="fa-solid fa-cross"></i>,
  },
  {
    obj: "To foster cooperation and love among the members and their families.",
    icon: <i className="fa-regular fa-heart"></i>,
  },
  {
    obj: "To organize educational and recreational activities for members and their families from time to time.",
    icon: <i className="fa-solid fa-graduation-cap"></i>,
  },
  {
    obj: "To cater for the welfare of the members.",
    icon: <i className="fa-solid fa-user-group"></i>,
  },
  {
    obj: "To take active part in all church activities",
    icon: <i className="fa-solid fa-church"></i>,
  },
  {
    obj: "To initiate and support developmental programmes in the church.",
    icon: <i className="fa-solid fa-handshake-angle"></i>,
  },
];

// export const members = [
//   { name: "Mr. Femi Kalesanwo", image: user1, },
//   { name: "Mr. Phillip Opadijo", image: user1, },
//   { name: "Mr. JK Diyaolu", image: user1, },
//   { name: "Mr. FI Agu", image: user1, },
//   { name: "Mr. JT Olatunde", image: user1, },
//   { name: "Mr. Osita Anigwe", image: user1, },
//   { name: "Olootu Patrick Olusegun Ogunfolaji", image: user1, }
// ];

export const members = [
  ["Mr. Femi Kalesanwo"],
  ["Mr. Phillip Opadijo"],
  ["Mr. JK Diyaolu"],
  ["Mr. FI Agu"],
  ["Mr. JT Olatunde"],
  ["Mr. Osita Anigwe"],
  ["Olootu Patrick Olusegun Ogunfolaji"],
];


export const assets = [
  {name:"Mr. George Opara",image:user1},
  {name:"Mr. Taiwo Ogunfolaji",image:user0},
  {name:"Mrs. Thomas Aregbesola",image:user2},
  {name:"Mrs. Comfort Opara",image:user3},
  {name:"Mr. Titicom Adegboyega",image:user4},
  {name:"Mr. Segun Adeniyi",image:user5},
  {name:"Mr. Michael Fatumbi",image:user6},
  {name:"Mr. and Mrs. Onabule",image:user7},
  {name:"Mr. and Mrs. Ogunfolaji",image:user8},
  {name:"Mr. and Mrs. Ogundipe",image:user9},
  {name:"Mr. and Mrs Fagbenro",image:user10},
  {name:"Mr. & Mrs. Oyibo",image:user11},
  {name:"Arctech and Mrs. Oluyide",image:user12}
]

export const current_exco = [
  {
    name:"Mr.George Diyaolu",
    image:cor1,
    position :"President"
  },
  {
    name:"Mr. Taiwo Ogunfolaji",
    image:user0,
    position :"Vice President"
  },
  {
    name:"Mr. Dennis Oni",
    image:cor2,
    position :"Active Member"
  },
  {
    name:"Mr. Francis Irepita",
    image:cor3,
    position :"Active Member"
  },
 
]